...
export function getTimeAgo (timestamp:string)  {
  ...
  // Update the state with the time difference in a human-readable format
...
export const saveCookieData = (data:DataType)=>{
  ...
  const secure = ENVIRONMENT_PROD ? true : false

  if(localKey === 'tenant')
  {
    cookies.set(localKey, value, { path: '/', secure:secure, sameSite:true,domain:COOKIE_DOMAIN});
    return
  }
  if(localKey === "refreshToken" || localKey === "accessToken" )
  {
    ...
    {
      cookies.set(localKey, value, { path: '/', secure:secure, expires:new Date(exp*1000),sameSite:true,domain:COOKIE_DOMAIN});
    }
